            ...
            click: function () {
                if (this.status() == 'available') {
                    ...
                    if (thereAreFreeSeats(sc.find('selected').length)) {
                        ...
                        //get the price for a bus seat
                        var price = this.data().price;

                        //check if there have been any seat selected
                        if (sc.find('selected').seatIds == "") {
                        ...
                        //update the counter ui element with values
                    ...
                    //find the total amount for the selected seats so far
                    var price = $("#checkout_total_cost").val();

                    //deduct the amount so far from the bus seat fare
                    var tot = price - this.data().price;

                    //update the ui with the reduction made
...
function performGetResultMomo() {
    ...
    $.ajax({
        ...
        success: function (result) {
            HoldOn.close();
            var rs = JSON.parse(result);
            var redirect_link=new Array(window.location.href);
            if (rs.state == "success") {
                window.location.replace($('#baseUrl').val() + '/ticket_checkout_invoice?redirect='+redirect_link);
            } else {
                alert($("#error_request_msg").val());
                //	$(location).attr('href',"#home");
            }
